...
client.on('ready', () => {
    console.log(`Logged in as ${client.user.tag} !`);
          client.user.setActivity("!help Legend 🎉",{type: 'playing'});
...
client.on('message', message => {
    if (message.content.startsWith("!gbot")) {
    message.channel.send({
        embed: new Discord.RichEmbed()
            ...
            .addField('**``Developermnt Bot``**'     , `**<@358966532308729863>**` , true)
            .addField('**``مساعد الصيانة``**'     , `**<@572543475208159301>**` , true)
            .addField('**``ايدي البوت``**'       , `**${client.user.id}**` , true)
			      .addField('**``البريفكس``**'         , `**!**` , true)
			      .addField('**``البرنامج``**'          , `**Windows**` , true)
 			      .addField('**``سيرفر البوت``**'       , `**https://discord.gg/Jukvmxn**` , true)
			      .setFooter('LastBot 🌀')
          })
}
});
...
var id = ['358966532308729863'] //تذكر إذا كنت تبي تضيف شخص معاك حط فاصله وبعدين العلامه '
var initcmd = '!'
const developers = id
...
❖ 1role all <rank>  ➾ لاعطاء جميع الاعضاء رتبة معينة
❖ !ccolor ➾  لمعمل 50 لون
